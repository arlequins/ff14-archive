<template>
  <div class="datepicker">
    <template v-if="hasInput">
      <input class="form-control datepicker-input" :class="{'with-reset-button': clearButton}" type="text" :placeholder="placeholder" :style="{width:width}" @click="inputClick" v-model="inputValue" />
            ...
            <div class="datepicker-monthRange">
              <template v-for="(m, $index) in text.months">
                <span :class="{'datepicker-dateRange-item-active':
                    ...
                    currDate.getFullYear() + pan === parse(value).getFullYear()}" @click="monthSelect(stringifyYearHeader(currDate, pan), $index)">{{m.substr(0,3)}}</span>
              </template>
            </div>
          </div>
        </div>
          ...
          <div class="datepicker-body">
            ...
            <div class="datepicker-monthRange decadeRange">
              <template v-for="decade in decadeRange[pan]">
                <span :class="{'datepicker-dateRange-item-active':
                    parse(inputValue).getFullYear() === decade.text}" @click.stop="yearSelect(decade.text)">{{decade.text}}</span>
              </template>
            </div>
          </div>
        </div>
...
export default {
  ...
  watch: {
    ...
    inputValue: function () {
      let targeting = this.inputValue.replace(/-/gi, '')
      this.$store.dispatch('setSearchingDate', targeting)
    },
    searchData: function () {
      if (this.searchData !== undefined) {
        if (this.searchData.hasOwnProperty('index')) {
          let data = this.searchData
          let year = data.index.endday.substring(0, 4)
          let month = data.index.endday.substring(4, 6)
          let day = data.index.endday.substring(6, 8)
  ...
  methods: {
    ...
    daySelect (date, event) {
      let el = event.target
      let selectDay = Number(this.stringify(date).replace(/-/gi, ''))
      if (this.searchData.hasOwnProperty('index')) {
        let startday = Number(this.searchData.index.startday)
        let endday = Number(this.searchData.index.endday)
        if (selectDay > endday || selectDay < startday) {
          alert(this.lp['season-error'][this.cl])
          return false
        }
      }
